/*!
  backgrid 0.3.7
  https://github.com/kirill-zhirnov/backgrid
      ...
      this.$el.find(':input').each(function (key, el) {
        ...
        (function ($el, self) {
          self.collection.queryParams[$el.attr('name')] = function () {
            var serializedVal = $el.serializeArray();
            if (serializedVal.length > 0 && serializedVal[0].value != '') {
              return serializedVal[0].value;
            }
  ...
  initialize: function (options) {
    ...
    if (collection.length == 0) {
      this.listenToOnce(collection, "sync", function () {
        self._markDefaultColumnOrder()
      });
    } else {
      this._markDefaultColumnOrder();
    }
  },

  _markDefaultColumnOrder : function() {
    if (this.collection.state.sortKey) {
      var column = this.columns.findWhere({name: this.collection.state.sortKey});
      if (column) {
